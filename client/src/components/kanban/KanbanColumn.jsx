  ...
  return (
    <section
      ...
      className={`
        ...
        backdrop-blur-sm
        transition-colors
        ${isOver ? "bg-[#0f0f0f]/90 border-white/40 ring-1 ring-white/30" : ""}
      `}
    >
      {/* Column header */}
      <header className="mb-2 flex h-9 items-center justify-between px-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="flex h-4.5 min-w-4.5 items-center justify-center rounded-full bg-[#111111] px-1.5 text-[10px] font-semibold text-[#8b8b92]">
            {tasks.length}
          </span>
        </div>
        ...
        >
          <div className="flex flex-col gap-2.5 pb-2 min-h-full">
            ...
            {tasks.length === 0 && (
              <div
                onClick={() => onAddTask && onAddTask(column.id)}
                className="flex flex-1 min-h-[100px] cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-[#1a1a1a] p-4 text-center transition hover:border-[#333333] hover:bg-white/[0.02]"
              >
